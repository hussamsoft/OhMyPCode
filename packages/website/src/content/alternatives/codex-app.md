...
Paseo creates worktrees under `$OHMYPCODE_HOME/worktrees`, runs setup and teardown hooks from `ohmypcode.json`, and gives each worktree its own dev server URLs like `web.fix-auth.my-app.localhost` so parallel services don't fight for ports.
