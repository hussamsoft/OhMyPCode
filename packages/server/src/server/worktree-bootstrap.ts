...
import type { PaseoServicePortAllocation } from "@ohmypcode/protocol/paseo-config-schema";
...
export async function spawnWorkspaceScript(
  ...
  if (!config) {
    throw new Error(`Script '${scriptName}' is not configured in ohmypcode.json`);
  }
