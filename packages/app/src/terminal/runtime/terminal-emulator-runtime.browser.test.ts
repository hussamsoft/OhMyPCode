...
function getBrowserTerminal(): BrowserTerminal {
  const terminal = window.__ompcodeTerminal as BrowserTerminal | undefined;
  if (!terminal) {
...
describe("terminal emulator runtime in a real browser", () => {
  it("passes configured scrollback to xterm", async () => {
    ...
    await waitFor({
      predicate: () => window.__ompcodeTerminal !== undefined,
    });

    expect(window.__ompcodeTerminal?.options.scrollback).toBe(42_000);
  });
  ...
  it("updates scrollback on the mounted xterm", async () => {
    ...
    const terminal = window.__ompcodeTerminal;

    mounted.runtime.setScrollback({ lines: 42_000 });

    expect(window.__ompcodeTerminal).toBe(terminal);
    expect(window.__ompcodeTerminal?.options.scrollback).toBe(42_000);
  });
