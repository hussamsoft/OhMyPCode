  ...
  while (pending.length) {
    ...
    for (const { fileName: specifier } of imports) {
      ...
      if (specifier.startsWith(".")) {
        ...
        if (specifier !== "zod" && specifier !== "@ohmypcode/protocol/agent-types")
          violations.push(label);
      } else if (runtime === "server") {
        if (uiDependency.test(specifier) || /^@ohmypcode\/plugin\/client(\/|$)/.test(specifier))
          violations.push(label);
      } else if (isBuiltin(specifier) || /^@ohmypcode\/plugin\/server(\/|$)/.test(specifier)) {
        violations.push(label);
      }
    }
  }
...
describe("plugin example import boundaries", () => {
  ...
  it.each(files)("%s respects runtime ownership, including type imports", (relative) => {
    ...
    for (const { fileName: specifier } of ts.preProcessFile(readFileSync(file, "utf8"), true, true)
      ...
      } else if (specifier.startsWith("@ohmypcode/plugin")) {
        const entry = specifier.replace("@ohmypcode/plugin", ".") as keyof typeof entries;
        if (
          entry === "./client/host" ||
          !(entry in entries) ||
