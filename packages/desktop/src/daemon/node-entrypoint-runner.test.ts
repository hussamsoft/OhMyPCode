...
declare global {
  ...
  var __OMPCODE_NODE_ENTRYPOINT_RUNNER_FIXTURE__:
    | {
        argv: string[];
...
describe("node-entrypoint-runner", () => {
  it("preserves Electron node-mode env before loading the target entrypoint", async () => {
    ...
    const originalFixture = globalThis.__OMPCODE_NODE_ENTRYPOINT_RUNNER_FIXTURE__;
    ...
    writeFileSync(
      fixturePath,
      `
globalThis.__OMPCODE_NODE_ENTRYPOINT_RUNNER_FIXTURE__ = {
  argv: process.argv,
...
`,
    ...
    delete globalThis.__OMPCODE_NODE_ENTRYPOINT_RUNNER_FIXTURE__;

    try {
      await main();

      expect(globalThis.__OMPCODE_NODE_ENTRYPOINT_RUNNER_FIXTURE__).toEqual({
      ...
      globalThis.__OMPCODE_NODE_ENTRYPOINT_RUNNER_FIXTURE__ = originalFixture;
