...
`packages/highlight/src/__tests__/dependency-closure.test.ts` replicates the packer's traversal statically and runs with the normal unit tests. It is scoped to `@ohmypcode/highlight` on purpose: that tree is small and pure, so the check is exact. Running the same walk over `@ohmypcode/server` produces dozens of false positives from optional dependencies loaded behind `try`/`catch`.
...
```bash
npm run test:e2e:browser-tabs --workspace=@ohmypcode/desktop
```
